...
vec3 getRay(vec2 fragCoord) {
  vec2 uv = ((fragCoord.xy / WindowSize.xy) * 2.0 - 1.0) * vec2(WindowSize.x / WindowSize.y, 1.0);
  vec3 proj = normalize(vec3(uv.x, uv.y, 1.0) + vec3(uv.x, uv.y, -1.0) * pow(length(uv), 2.0) * 0.05);  
  if(WindowSize.x < 600.0) {
    return proj;
  }
...
void main()
{
  ...
  vec3 origin = vec3(time * 0.2, CAMERA_HEIGHT, 1);
